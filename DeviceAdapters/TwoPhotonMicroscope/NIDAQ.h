...
public:
    ...
    int StartDOBlankingAndOrSequenceWithoutTrigger(const std::string& port, const bool blankingOn,
        const bool sequenceOn, const long& pos, const bool blankingDirection, int32 num, double SampleRate, int state);
    ...
    static int32 CVICALLBACK TaskDoneCallback(TaskHandle taskHandle, int32 status, void* callbackData);
    static int32 CVICALLBACK TaskStartCallback(TaskHandle taskHandle, int32 everyNsamplesEventType, uInt32 nSamples, void* callbackData);
...
private:
    ...
    TaskHandle doTask_;
    TaskHandle diTask_2;
    TaskHandle PixelTrigger_;
    TaskHandle LineTrigger_;

    std::string PixelClockPort = "Dev1/port0/line0";
    std::string LineClockPort = "Dev1/port0/line1";
    std::string FrameClockPort = "Dev1/port0/line2";
...
public:
    ...
    virtual int StartAISequenceForPort(const std::string& port, const std::string& triggerPort_, const double SampleRate, const double resolution);
    static int32 CVICALLBACK AITaskDoneCallback(TaskHandle taskHandle, int32 status, void* callbackData);

    virtual int IsSequencingEnabled(bool& flag) const;
    virtual int GetSequenceMaxLength(long& maxLength) const;

    int AddDOPortToSequencing(const std::string& port, const std::vector<uInt32> sequence);
    ...
    int SetLineClockPortState(int itit);
    int SetFrameClockPortState(int itit);
    ...
    float64 aidata_galvo[512 * 512];
    float64 aidata_resonanty[512];
    bool done;
    bool complete = false;
    std::vector<float64> data;  // 用于存储读取的数据
    int32 dataSize; // 用于存储读取的数据大小
...
private:
    ...
    int StartAISequencingTask(const std::string& port, const std::string& triggerPort_, const double SampleRate, const double resolution);
